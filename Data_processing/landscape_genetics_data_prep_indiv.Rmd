...
# Function to convert to allelic freq. with random ref
...
# loadings matrix 
  ...
  col_alLD <- apply(col_gt, 2, allele_load)
...
# extract proportion of forest within 500 m of a plant
  plant_coords <- as.matrix(
    samp_dat_sub[,c(which(names(samp_dat_sub)%in%c("X_COORD", "Y_COORD")))]
    )
  plant_connect <- 
    extract(forest, plant_coords, fun=mean, na.rm=T, buffer=500, df=T)
  
  samp_dat_sub$PROP_FOREST_500M_PL <- plant_connect$forest
```
...
# create remaining slices
 ...
 # which are covariates pertaining to the "sending" meadow?
  rem_slices <- c("PL_IN_5M", "COVER", "PROP_FOREST_500M_PL")
